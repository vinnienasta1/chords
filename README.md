...
1) **Сайт (PHP, LXC 108)** — фронтенд/админка, общается с БД и Telegram‑ботом.  
2) **База данных (MySQL/MariaDB, LXC 109)** — хранит пользователей, песни, сеты и заявки верификации.  
3) **Telegram‑бот (Python + Flask API, LXC 110)** — верификация пользователей, приём/хеширование паролей, рассылка.
...
3. Убедитесь, что сервер БД доступен с LXC 108 и 110.

## Настройка сайта (LXC 108)
...
## Настройка Telegram‑бота (LXC 110)
...
2. Создайте `config.py` из `config.example.py` и заполните:
   ...
   - `DB_HOST/DB_PORT/DB_USER/DB_PASSWORD/DB_NAME` — доступ к БД (LXC 109).  
...
- **Часовой пояс/UTC**: следите, чтобы время на всех LXC было синхронизировано — заявки/TTL завязаны на `expires_at`.  
...
1. БД поднята, схема импортирована, доступ с 108/110 есть.  
